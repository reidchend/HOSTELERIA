...
class Reservacion(Base):
    ...
    # Control
    estado = Column(Enum(EstadoReservacion), default=EstadoReservacion.PENDIENTE)
...
class BitacoraEvento(Base):
    ...
    telegram_message_id → ID del mensaje de Telegram (para responder/vincular pagos)
    """
...
class Turno(Base):
    ...
    eventos = relationship(
        ...
        order_by="BitacoraEvento.creado_en",
        lazy="dynamic",
    )
    cuentas_pagar = relationship(
        "CuentaPagar",
        back_populates="turno",
