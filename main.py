...
def principal(pagina: ft.Page):
    ...
    _zona_contenido = ft.Container(
        expand=True, animate_opacity=300, content=ft.ProgressRing(visible=False)
    )
    _sidebar_ref = [None]
    ...
    def _mostrar_dialogo_limpieza(habitacion):
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Text(
                "¿Confirmar que la habitación está lista para recibir huéspedes?"
            ),
            actions=[
    ...
    def iniciar_checkout(habitacion):
        ...
        CheckOutWizard(
            pagina, habitacion, al_completar=lambda _: refrescar_grid()
        ).mostrar()
    ...
    class Sidebar(ft.Container):
        ...
        def _construir(self):
            ...
            self.animate_size = ft.Animation(
                duration=450, curve=ft.AnimationCurve.EASE_OUT_QUINT
            )
            ...
            self._header_container = ft.Container(
                content=ft.Row(
                    [
                        ft.Row(
                            [self._logo_badge, self._logo_texto],
                            spacing=10,
                            tight=True,
                            visible=self._expandido,
                        ),
                ...
                padding=ft.padding.only(
                    left=12 if self._expandido else 0,
                    right=4 if self._expandido else 0,
                    top=20,
                    bottom=20,
                ),
            ...
            self._footer_container = ft.Container(
                ...
                padding=ft.padding.symmetric(
                    horizontal=12 if self._expandido else 0, vertical=15
                ),
            )
            ...
            self.content = ft.Column(
                [
                    ...
                    ft.Column(
                        [self._nav_item(v, l, i) for v, l, i in nav_items],
                        spacing=4,
                        expand=True,
                    ),
                ...
                content=ft.Row(
                    [
                        ...
                        ft.Text(
                            label,
                            size=13,
                            weight="w600" if es_activo else "w400",
                            color=ft.Colors.ON_SURFACE
                            if es_activo
                            else ft.Colors.ON_SURFACE_VARIANT,
                            visible=self._expandido,
                    ...
                    spacing=12 if self._expandido else 0,
                    alignment=ft.MainAxisAlignment.CENTER
                    if not self._expandido
                    else ft.MainAxisAlignment.START,
                ),
                padding=ft.padding.symmetric(
                    horizontal=12 if self._expandido else 0, vertical=10
                ),
        ...
        def _toggle_colapso(self, _):
            ...
            for item in self._items_refs.values():
                ...
                item.margin = ft.margin.symmetric(
                    horizontal=8 if self._expandido else 4
                )
            ...
            # Ajustar Header
            self._header_container.content.alignment = (
                ft.MainAxisAlignment.SPACE_BETWEEN
                if self._expandido
                else ft.MainAxisAlignment.CENTER
            )
            self._header_container.padding = ft.padding.only(
                left=12 if self._expandido else 0,
                right=4 if self._expandido else 0,
                top=20,
                bottom=20,
            ...
            # Ajustar Footer
            self._footer_container.content.alignment = (
                ft.MainAxisAlignment.START
                if self._expandido
                else ft.MainAxisAlignment.CENTER
            )
            self._footer_container.padding = ft.padding.symmetric(
                horizontal=12 if self._expandido else 0, vertical=15
            )
    ...
    def _construir_topbar():
        ...
        nombre_u = (
            estado_app["usuario_activo"].get("nombre_completo", "Admin")
            if estado_app["usuario_activo"]
            else "User"
        )
            ...
            [
                ...
                ft.Column(
                    [
                        _construir_topbar(),
                        ft.Container(
                            content=_zona_contenido, expand=True, bgcolor=None
                        ),
                    ],
                    expand=True,
                    spacing=0,
    ...
    def cerrar_sesion():
        ...
        if id_turno:
            ...
            DialogoCierreTurno(
                pagina=pagina,
                id_turno=id_turno,
                al_cerrar_turno=al_cerrar_turno,
            ).mostrar()
        else:
            estado_app["usuario_activo"] = None
            mostrar_login()
